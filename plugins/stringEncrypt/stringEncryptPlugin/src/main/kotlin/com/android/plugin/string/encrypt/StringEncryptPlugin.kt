 ...
 * create on 2022/7/5
 */
class StringEncryptPlugin : Transform(), Plugin<Project> {

    private val getTaskNativeLibs = Regex("^merge\\S*NativeLibs\$")
    private val oldFilename = "liba.so"
    private val oldFilename2 = "libchecknative.so"
    private val oldFilename3 = "libnativetools.so"

    private val newFilename = RandomUtil.genRandomStr()
    private val newFilename2 = RandomUtil.genRandomStr()
    private val newFilename3 = RandomUtil.genRandomStr()

    private val needModifySoNameClass = "com/android/lib/string/decrypt/HexDecryptUtils.class"
    private val needModifySoNameClass2 = "androidx/security/crypto/utils/RiskLoadUtil.class"
    private val needModifySoNameClass3 = "com/android/lib/NativeToolsLib.class"
    ...
    override fun apply(project: Project) {
        ...
        project.afterEvaluate {
            ...
            logEnable=ext?.logEnable==true
            log("afterEvaluate,ext=$ext")
            findAndReplaceSoName(it)
        }
    }
     ...
     */
    private fun findAndReplaceSoName(project: Project) {
        val allTasks = project.getAllTasks(true)
        allTasks.entries.forEach {
            val value = it.value
            value.forEach { task ->
                log("task name>>> ${task.name}")
                if (task.name.matches(getTaskNativeLibs)) {

                    task.doLast { tasklast ->
                        tasklast.outputs.files.forEach { file ->
    ...
    private fun getSoFile(file: File) {
        file.listFiles()?.forEach {
            if (it.isDirectory) {
                getSoFile(it)
            } else if (it.name == oldFilename) {
                log("task outputfiles >>> ${it.name}")
                reNameSoFile(it,newFilename)
            }else if(it.name == oldFilename2){
                log("task outputfiles >>> ${it.name}")
                reNameSoFile(it,newFilename2)
            } else if(it.name == oldFilename3) {
                log("task outputfiles >>> ${it.name}")
                reNameSoFile(it,newFilename3)
            } else{
                log("task outputfiles >>> ${it.name}")
            }
        }
    ...
    private fun reNameSoFile(file: File?, newSoname: String) {
        if (file == null) return

        val abs_filePath = file.absolutePath
        val filePath = abs_filePath.substring(0, abs_filePath.lastIndexOf(File.separator))
        val newFilePath =  "${filePath}${File.separator}lib${newSoname}.so"

        val renameTo = file.renameTo(File(newFilePath))
        if (renameTo) {
            log("renameTo success >>> ${file.absolutePath}")
        }
    ...
    private fun isClassNeedModifyForDir(name: String): Boolean {
    ...
    private fun isClassNeedModifyForJar(name: String): Boolean {
        if(isWindows()) {
            ext?.encryptPackages?.forEach { pkg ->
                if (name.contains(pkg.replace(".", "/"))) {
                    log("$name need modify-string-encrypt")
                    return true
                }
            }
    ...
    private fun modifyClassIfNeeded(dir: File, file: File) {
        ...
        log("modifyClassIfNeeded,getAllFiles file=${clzName}")
        if (!isClassNeedModifyForDir(clzName)) {
            return
        }
    ...
    private fun modifyJarIfNeeded(oriJarPath: File, destJar: File) {
        ...
        while (entries.hasMoreElements()) {
            ...
            val isNeedModify = isClassNeedModifyForJar(entry.name)
            ...
            } else if (isClassFile(entry.name)) {
                val newBytes = modifyClassByAddingFieldOrRenameLDC(entry.name, oriBytes)
                val toWriteBytes = newBytes ?: oriBytes
                destFos.write(toWriteBytes, 0, toWriteBytes.size)
            } else {
    ...
    private fun modifyClassByAddingFieldOrRenameLDC(name: String, bytes: ByteArray?): ByteArray? {
        log("modifyClassByAddingField,name=$name")
        bytes ?: return null
        return try {
            val cr = ClassReader(bytes)
            val cw = ClassWriter(cr, ClassWriter.COMPUTE_MAXS)
            val cv = if (name == needModifySoNameClass)
                ModifySoNameClassVisitor(
                newFilename,
                cw)
            else if (name == needModifySoNameClass2)
                ModifySoNameClassVisitor(
                newFilename2,
                cw)
            else if (name == needModifySoNameClass3)
                ModifySoNameClassVisitor(
                    newFilename3,
                    cw)
            else AddFieldClassVisitor(cw)
            cr.accept(cv, ClassReader.EXPAND_FRAMES)
            cw.toByteArray() ?: bytes
        } catch (e: Throwable) {
            e.printStackTrace()
    ...
    override fun transform(transformInvocation: TransformInvocation) {
        ...
        val isReleaseBuild =
            transformInvocation.context.variantName.toLowerCase(Locale.ROOT).contains("release")
            ...
            it.directoryInputs.forEach { dirInput ->
                ...
                if (enable) {
                    dirInput.file.eachFileRecurse { file ->
                        log("getAllFiles ,name =${file.name}")
