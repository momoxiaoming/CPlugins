...
object Base64 {
    ...
    /**
     * Encoder flag bit to omit the padding '='.toByte() characters at the end
    ...
    internal abstract class Coder {
         ...
         * be an overestimate.
         */
        protected abstract fun maxOutputSize(len: Int): Int
    }
     ...
     *
     *
     * The padding '='.toByte() characters at the end are considered optional, but
     ...
     * incorrect padding
     */
    private fun decode(input: ByteArray, offset: Int, len: Int, flags: Int): ByteArray? {
    ...
    private class Decoder private constructor(flags: Int, output: ByteArray) :
        Coder() {
        /**
         * States 0-3 are reading through the next input tuple.
         * State 4 is having read one '='.toByte() and expecting exactly
         * one more.
         ...
         */
        public override fun maxOutputSize(len: Int): Int {
            return len * 3 / 4 + 10
        }
        ...
        override fun process(input: ByteArray, offset: Int, len: Int, finish: Boolean): Boolean {
            ...
            while (p < len) {
                ...
                if (state == 0) {
                    while (p + 4 <= len &&
                        (alphabet[input[p] and 0xff] shl 18 or
                                (alphabet[input[p + 1] and 0xff] shl 12) or
                                (alphabet[input[p + 2] and 0xff] shl 6) or
                                alphabet[input[p + 3] and 0xff]).also { value = it } >= 0
                    ) {
            ...
            when (state) {
                ...
                1, 4 -> {
                    // Read one padding '='.toByte() when we expected 2.  Illegal.
     ...
     * adheres to RFC 2045.
     */
    fun encode(str: String, flags: Int): ByteArray? {
        return encode(str.toByteArray(), flags)
    }
     ...
     * adheres to RFC 2045.
     */
    private fun encode(input: ByteArray, offset: Int, len: Int, flags: Int): ByteArray? {
    ...
    private class Encoder private constructor(flags: Int, output: ByteArray?) :
        Coder() {
        ...
        override fun process(input: ByteArray, offset: Int, len: Int, finish: Boolean): Boolean {
            ...
            if (finish) {
                ...
                if (p - tailLen == len - 1) {
                    ...
                    var t = 0
                    v = (if (tailLen > 1) tail[t++] else input[p++]) and 0xff shl 10 or
                            ((if (tailLen > 0) tail[t++] else input[p++]) and 0xff shl 2)
                    tailLen -= t
                    output[op++] = alphabet[v shr 12 and 0x3f]
                    output[op++] = alphabet[v shr 6 and 0x3f]
                    output[op++] = alphabet[v and 0x3f]
    ...
    private fun Base64() {} // don't instantiate
